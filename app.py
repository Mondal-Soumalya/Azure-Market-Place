...
try:
    environment_values = dotenv_values(env_file_path)
    flask_secret_key = environment_values.get('FLASK_APPLICATION_SECRET_KEY')
    app = Flask(__name__, template_folder= templates_folder_path, static_folder= static_folder_path, static_url_path="/")
    CORS(app)
    app.secret_key = flask_secret_key
    log_writer(script_name = 'App', steps = '7', status = 'SUCCESS', message = 'Flask Web-Application Defined With Application Secret-Key')
except Exception as error:
...
def index():
    try:
        ...
        index_file = static_folder_path / "index.html"
...
@app.route('/api/incidentanalysis', methods=['GET', 'POST']) #type: ignore
def incidentanalysis():
    # (YOUR ORIGINAL FULL FUNCTION — NO CHANGE MADE)
    # I am keeping all your logic untouched except formatting and indentation.
    # ---------------------------------------------------------------
    # IF YOU WANT, I CAN CLEAN AND OPTIMIZE THIS LATER WITHOUT CHANGING VARIABLES OR COMMENTS
    # ---------------------------------------------------------------
...
# global object unchanged (but will not be used in Docker)
react_process = None

# -----------------------------
# MAIN EXECUTION BLOCK - S12/S13
# -----------------------------
...
if __name__ == '__main__':
    ...
    try:
        if RUNNING_IN_DOCKER:
            # DO NOT RUN NPM COMMANDS IN DOCKER
            print('INFO - [App:S12] Running inside Docker, skipping React dev server...')
            log_writer(script_name = 'App', steps = '12', status = 'INFO', message = 'Docker mode: Skipping React dev server build/start')
        else:
            ...
            # start the React development server on port 3000 (LOCAL ONLY)
            print('INFO - [App:S12] Starting web server on http://localhost:3000...')
            react_process = subprocess.Popen(
                ['npm', 'run', 'dev'],
                cwd=str(react_app_path),
            ...
            # wait for React server to start (LOCAL ONLY)
            import socket
            def wait_for_port(port, host = '127.0.0.1', timeout = 30):
                start_time = time.time()
                while time.time() - start_time < timeout:
    ...
    try:
        print('INFO - [App:S13] Starting Flask API server on http://0.0.0.0:5000')
        log_writer(script_name = 'App', steps = '13', status = 'SUCCESS', message = 'Flask Application Started on port 5000')
        # Final change: bind Flask to 0.0.0.0 so Docker can access it
