        ...
        with psycopg2.connect(**database_connection_parameter) as database_connection: #type: ignore
            with database_connection.cursor() as database_cursor:
                ...
                if (database_cursor.fetchone()[0]):
                    ...
                    database_cursor.execute("SELECT COUNT(*) FROM skip_row_details;")
                    if int(database_cursor.fetchone()[0]) == 0:
                        database_cursor.execute("DROP TABLE skip_row_details;")
                        database_connection.commit()
                    else:
                        return {'status': 'SUCCESS', 'file_name': 'Skip-Row-Details-Table-Create', 'step': '3', 'message': '"skip_row_details" Table Already Present With Data'}
    except Exception as error:
