...
def incident_ticket_file_handler(user_email: str, user_unique_id: str, account_unique_id: str, file) -> dict[str, str]: #type: ignore
    ...
    try:
        ...
        incident_files_folder_path = Path(submitted_files_folder_path) / 'IncidentFiles'
        temp_files_dump_folder_path = Path(parent_folder_path) / 'TempFilesDump'
        log_writer(script_name = 'Incident-File-Handler', steps = '4', status = 'SUCCESS', message = 'All Folders And Files Path Are Defined')
    except Exception as error:
    ...
    try:
        if ((env_file_path.exists()) and (env_file_path.is_file())):
            ...
            log_writer(script_name = 'Incident-File-Handler', steps = '6', status = 'ERROR', message = '".env" File Not Present')
            return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '6', 'message' : '".env" File Not Present'}
    except Exception as error:
    ...
    try:
        ...
        log_writer(script_name = 'Incident-File-Handler', steps = '8', status = 'SUCCESS', message = 'Database Connection Parameter Defined')
    except Exception as error:
        ...
        with psycopg2.connect(**database_connection_parameter) as database_connection: #type: ignore
            with database_connection.cursor() as database_cursor:
                ...
                if (database_cursor.fetchone()[0]):
                    log_writer(script_name = 'Incident-File-Handler', steps = '9', status = 'SUCCESS', message = '"submitted_file_details" Table Present Inside Database')
                else:
                    log_writer(script_name = 'Incident-File-Handler', steps = '9', status = 'ERROR', message = '"submitted_file_details" Table Not Present')
                    return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '9', 'message' : '"submitted_file_details" Table Not Present'}
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '9', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '9', 'message' : str(error)}
    ...
    try:
        ...
        with psycopg2.connect(**database_connection_parameter) as database_connection: #type: ignore
            with database_connection.cursor() as database_cursor:
                ...
                if (database_cursor.fetchone()[0]):
                    log_writer(script_name = 'Incident-File-Handler', steps = '10', status = 'SUCCESS', message = '"account_details" Table Present Inside Database')
                else:
                    log_writer(script_name = 'Incident-File-Handler', steps = '10', status = 'ERROR', message = '"account_details" Table Not Present')
                    return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '10', 'message' : '"account_details" Table Not Present'}
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '10', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '10', 'message' : str(error)}
    ...
    try:
        ...
        log_writer(script_name = 'Incident-File-Handler', steps = '11', status = 'SUCCESS', message = f'"{new_file_name}" File Name Created')
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '11', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '11', 'message' : str(error)}
    ...
    try:
        if (file.filename.endswith('.xlsx')):
            ...
            log_writer(script_name = 'Incident-File-Handler', steps = '12', status = 'SUCCESS', message = f'"{new_file_name}" File Saved In "../IncidentFiles" Folder')
        else:
            log_writer(script_name = 'Incident-File-Handler', steps = '12', status = 'ERROR', message = f'"{new_file_name}" Doesn\'t Has Allowed Extension')
            return {'status' : 'INFO', 'file_name' : 'Incident-File-Handler', 'step' : '12', 'message' : 'Submitted File Type Not In ".xlsx" Format'}
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '12', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '12', 'message' : str(error)}
    ...
    try:
        ...
        if ((int(new_file_archive_store_path.stat().st_size)) == (int(new_file_temp_stored_path.stat().st_size))):
            log_writer(script_name = 'Incident-File-Handler', steps = '13', status = 'SUCCESS', message = f'"{new_file_name}" File Copied To "../TempFilesDump" Folder For Future Processing')
        else:
            log_writer(script_name = 'Incident-File-Handler', steps = '13', status = 'ERROR', message = f'"{new_file_name}" File Not Copied To "../TempFilesDump" Folder')
            return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '13', 'message' : f'"{new_file_name}" File Not Copied To "../TempFilesDump" Folder'}
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '13', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '13', 'message' : str(error)}
    ...
    try:
        ...
        log_writer(script_name = 'Incident-File-Handler', steps = '14', status = 'SUCCESS', message = f'For File: "{shared_file_name}" All Metadata Fetched For Data Insertion')
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '14', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '14', 'message' : str(error)}
    ...
    while True:
        ...
        try:
            ...
            log_writer(script_name = 'Incident-File-Handler', steps = '15', status = 'SUCCESS', message = f'For File: "{shared_file_name}" New "file_unique_id" Generated')
        except Exception as error:
            log_writer(script_name = 'Incident-File-Handler', steps = '15', status = 'ERROR', message = str(error))
            return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '15', 'message' : str(error)}
            ...
            with psycopg2.connect(**database_connection_parameter) as database_connection: #type: ignore
                with database_connection.cursor() as database_cursor:
                    ...
                    if (database_cursor.fetchone()):
                        log_writer(script_name = 'Incident-File-Handler', steps = '16', status = 'INFO', message = f'For File: "{shared_file_name}" Generated "file_unique_id" Already Present Inside "file_submitted_details" Table, Hence Generating New "file_unique_id"')
                        continue
                    else:
                        log_writer(script_name = 'Incident-File-Handler', steps = '16', status = 'SUCCESS', message = f'For File: "{shared_file_name}" Generated "file_unique_id" Not Present Inside "file_submitted_details" Table')
                        break
        except Exception as error:
            log_writer(script_name = 'Incident-File-Handler', steps = '16', status = 'ERROR', message = str(error))
            return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '16', 'message' : str(error)}
    ...
    try:
        ...
        log_writer(script_name = 'Incident-File-Handler', steps = '17', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '17', 'message' : str(error)}
    ...
    try:
        ...
        with psycopg2.connect(**database_connection_parameter) as database_connection: #type: ignore
            with database_connection.cursor() as database_cursor:
                database_cursor.execute(file_metadata_check_for_submitted_file_details_sql, (str(account_unique_id), str(file_unique_id), str(user_unique_id), str(saved_file_name)))
                if (database_cursor.fetchone()[0]):
                    log_writer(script_name = 'Incident-File-Handler', steps = '18', status = 'SUCCESS', message = f'For File: "{shared_file_name}" All Metadata Stored Inside "submitted_file_details" Table')
                else:
                    log_writer(script_name = 'Incident-File-Handler', steps = '18', status = 'ERROR', message = f'For File: "{shared_file_name}" Metadata Not Stored Inside "submitted_file_details" Table')
                    return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '18', 'message' : f'For File: "{shared_file_name}" Metadata Not Stored Inside "submitted_file_details" Table'}
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '18', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '18', 'message' : str(error)}
        ...
        if (int(len(temp_file_sheet_names)) == 1):
            ...
            with psycopg2.connect(**database_connection_parameter) as database_connection: #type: ignore
                with database_connection.cursor() as database_cursor:
                    ...
                    database_connection.commit()
                    log_writer(script_name = 'Incident-File-Handler', steps = '19', status = 'SUCCESS', message = f'For Account: "{account_unique_id}" Total Rows Count Updated Inside "account_details" Table')
        else:
            log_writer(script_name = 'Incident-File-Handler', steps = '19', status = 'INFO', message = f'For Account: "{account_unique_id}" Skipping "total_rows_count" Update Inside "account_details" Table Due To Not Approved File')
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '19', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '19', 'message' : str(error)}
    ...
    #                 <p><span style="color: green; font-weight: bold;"><strong>Success!</strong></span> The incident dump file has been uploaded <em>without any issues</em>.</p>
    ...
    # except Exception as error:
    #     log_writer(script_name = 'Incident-File-Handler', steps = '20', status = 'ERROR', message = str(error))
    #     return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '20', 'message' : str(error)}

    # calling "incident_data_process" thread:S21
    try:
        ...
        thread.start()
        log_writer(script_name = 'Incident-File-Handler', steps = '21', status = 'SUCCESS', message = 'A New Thread Started For Incident Data Processing')
        # return message to frontend
        return {'status' : 'SUCCESS', 'file_name' : 'Incident-File-Handler', 'step' : '21', 'message' : 'File Saved & Data Processing Will Start Shortly'}
    except Exception as error:
        log_writer(script_name = 'Incident-File-Handler', steps = '21', status = 'ERROR', message = str(error))
        return {'status' : 'ERROR', 'file_name' : 'Incident-File-Handler', 'step' : '21', 'message' : str(error)}