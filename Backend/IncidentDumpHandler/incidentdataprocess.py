...
def incident_data_process(account_unique_id: str, file_unique_id: str, user_unique_id: str, user_email: str, file_path: str):
    ...
    # INCIDNET_DATA_FILE_PROCESS_STATUS_UPDATE_PROCESS_COMPLETE_STATUS = False
    ...
    try:
        ...
        # error_mail(file_name = 'Incident-Data-Process', step = '10-A', message = str(error))
    ...
    try:
        ...
        # error_mail(file_name = 'Incident-Data-Process', step = '10-B', message = str(error))
    ...
    #         if (int(len(incident_column_process_backend_response['missing_columns'])) == 0):
    #             # define html body
    #             html_body = f'''<p><strong>Initial processing is complete.</strong> And we faced: <span style="color: red; font-weight: bold;">{incident_column_process_backend_response['message']}</span></p><br>'''

    #         # check if "missing_columns" not empyt
    #         if (int(len(incident_column_process_backend_response['missing_columns'])) > 0):
    #             # define html body empty string
    #             html_body = f'''<p><strong>Initial processing is complete.</strong> And we faced: <span style="color: red; font-weight: bold;">{incident_column_process_backend_response['message']}</span></p>'''
    #             for col in incident_column_process_backend_response['missing_columns']:
    ...
    #         log_writer(script_name = 'Incident-Data-Process', steps = '10-C', status = 'SUCCESS', message = f'Email Sent To User: "{str(user_email).lower()}" With Proper Description')
    # except Exception as error:
    #     log_writer(script_name = 'Incident-Data-Process', steps = '10-C', status = 'ERROR', message = str(error))
    #     error_mail(file_name = 'Incident-Data-Process', step = '10-C', message = str(error))
    ...
    if (str(incident_column_process_backend_response['status']).lower() == 'error'):
        try:
            ...
            # error_mail(file_name = str(incident_column_process_backend_response['file_name']), step = str(incident_column_process_backend_response['step']), message = str(incident_column_process_backend_response['message']))
        except Exception as error:
            log_writer(script_name = 'Incident-Data-Process', steps = '10-D', status = 'ERROR', message=str(error))
            # error_mail(file_name = 'Incident-Data-Process', step = '10-D', message = str(error))
    ...
    if (str(incident_column_process_backend_response['status']).lower() == 'info'):
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '10-E', message = str(error))
    ...
    if (str(incident_column_process_backend_response['status']).lower() == 'success'):
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '10-F', message = str(error))
    ####################################################
    ...
    if (INCIDENT_DATA_COLUMN_PROCESS_COMPLETE_STATUS):
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '11-A', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '11-B', message = str(error))
        ...
        try:
            if (str(incident_data_file_to_db_backend_response['status']).lower() == 'error'):
                INCIDENT_DATA_FILE_TO_DB_PROCESS_COMPLETE_STATUS =False
                # sending error mail to developer
                # error_mail(file_name = str(incident_data_file_to_db_backend_response['file_name']), step = str(incident_data_file_to_db_backend_response['step']), message = str(incident_data_file_to_db_backend_response['message']))
                # converting "file_path" to Path object
                file_path_object = Path(file_path)
                # delete "file_path"
                file_path_object.unlink()
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '11-C', message = str(error))
        ...
        if (str(incident_data_file_to_db_backend_response['status']).lower() == 'success'):
            ...
            try:
                ...
                # error_mail(file_name = 'Incident-Data-Process', step = '11-D', message = str(error))

            # # importing "create_ticket" function:S11-E
            # try:
            #     from Backend.SnowHandler.createticket import create_ticket
            # except Exception as error:
            #     log_writer(script_name = 'Incident-Data-Process', steps = '11-E', status = 'ERROR', message = str(error))
            #     error_mail(file_name = 'Incident-Data-Process', step = '11-E', message = str(error))

            # # calling "create_ticket" function:S11-F
            # try:
            #     servicenow_ticket_create_response = create_ticket(short_description = 'Incident Ticket Analysis With PRiSM Analytics')
            #     if (servicenow_ticket_create_response != None):
            #         log_writer(script_name = 'Incident-Data-Process', steps = '11-F', status = 'SUCCESS', message = f'For File: "{Path(file_path).name}" New ServiceNow Ticket Created')
            # except Exception as error:
            #     log_writer(script_name = 'Incident-Data-Process', steps = '11-F', status = 'ERROR', message = str(error))
            #     error_mail(file_name = 'Incident-Data-Process', step = '11-F', message = str(error))

            # # if servicenow response is "ERROR":S11-G
            # if (str(servicenow_ticket_create_response['status']).lower() == 'error'):
            #     try:
            #         INCIDENT_DATA_FILE_TO_DB_PROCESS_COMPLETE_STATUS = False
            #         log_writer(script_name = 'Incident-Data-Process', steps = '11-G', status = 'SUCCESS', message = f'For File: "{Path(file_path).name}" ServiceNow Ticket Not Created Because Of Technical Issue')
            #         # define email body
            #         email_body = f'''
            #         <html>
            #             <body>
            #                 <p>Dear Team,</p>
            #                 <p>The incident dump file has been uploaded successfully.</p>
            ...
            #         # sending the email
            #         mail_handler(email_subject = 'ASTRA | ESOAR Analysis', email_to = str(user_email).lower(), email_cc = 'soumalya.mondal@capgemini.com;anurag.thakur@capgemini.com', email_body = str(email_body))
            #         log_writer(script_name = 'Incident-Data-Process', steps = '11-G', status = 'INFO', message = f'Email Sent To User: "{str(user_email).lower()}" With No ServiceNow Ticket Details')
            #     except Exception as error:
            #         log_writer(script_name = 'Incident-Data-Process', steps = '11-G', status = 'ERROR', message = str(error))
            #         error_mail(file_name = 'Incident-Data-Process', step = '11-G', message = str(error))

            # # if servicenow resposne "SUCCESS":S11-H
            # if (str(servicenow_ticket_create_response['status']).lower() == 'success'):
            #     try:
            #         log_writer(script_name = 'Incident-Data-Process', steps = '11-H', status = 'SUCCESS', message = f'''New ServiceNow Ticket Created: "{servicenow_ticket_create_response['ticket_number']}" For File: "{Path(file_path).name}"''')
            #         # define email body
            #         email_body = f'''
            #         <html>
            #             <head>
            ...
            #         # sending the email
            #         mail_handler(email_subject = 'ASTRA | ESOAR Analysis', email_to = str(user_email).lower(), email_cc = 'soumalya.mondal@capgemini.com;anurag.thakur@capgemini.com', email_body = str(email_body))
            #         log_writer(script_name = 'Incident-Data-Process', steps = '11-H', status = 'SUCCESS', message = f'''Email Sent To User: "{str(user_email).lower()}" With ServiceNow Ticket: "{servicenow_ticket_create_response['ticket_number']}" Details''')
            #     except Exception as error:
            #         log_writer(script_name = 'Incident-Data-Process', steps = '11-H', status = 'ERROR', message = str(error))
            #         error_mail(file_name = 'Incident-Data-Process', step = '11-H', message = str(error))
                ...
                # except Exception as error:
                #     log_writer(script_name = 'Incident-Data-Process', steps = '11-I', status = 'ERROR', message = str(error))
                #     error_mail(file_name = 'Incident-Data-Process', step = '11-I', message = str(error))
                ...
                #             if (database_cursor.fetchone()[0]):
                #                 log_writer(script_name = 'Incident-Data-Process', steps = '11-J', status = 'SUCCESS', message = f'For File: "{file_unique_id}" File Status, ITSM Ticket Number, ITSM Global ID Updated Inside "submitted_file_details" Table')
                #             else:
                #                 log_writer(script_name = 'Incident-Data-Process', steps = '11-J', status = 'ERROR', message = f'For File: "{file_unique_id}" File Status, ITSM Ticket Number, ITSM Global ID Not Updated Inside "submitted_file_details" Table')
                #                 error_mail(file_name = 'Incident-Data-Process', step = '11-J', message = f'For File: "{file_unique_id}" File Status, ITSM Ticket Number, ITSM Global ID Not Updated Inside "submitted_file_details" Table')
                # except Exception as error:
                #     log_writer(script_name = 'Incident-Data-Process', steps = '11-J', status = 'ERROR', message = str(error))
                #     error_mail(file_name = 'Incident-Data-Process', step = '11-J', message = str(error))

                # # updaing worknotes accordingly:S11-K
                # try:
                #     elapsed_seconds = int((incident_file_to_db_process_end_time - incident_file_to_db_process_start_time) % 60) #type: ignore
                #     elapsed_minutes = int(((incident_file_to_db_process_end_time - incident_file_to_db_process_start_time) % 3600) // 60) #type: ignore
                #     elapsed_hours = int((incident_file_to_db_process_end_time - incident_file_to_db_process_start_time) // 3600) #type: ignore
                ...
                #         INCIDENT_DATA_FILE_TO_DB_PROCESS_COMPLETE_STATUS = True
                #         log_writer(script_name = 'Incident-Data-Process', steps = '11-K', status = 'SUCCESS', message = f'For Account: "{str(account_unique_id)}" Transferring Ticket From File To Database Process Completed And Worknotes Updated In ServiceNow')
                #     else:
                #         INCIDENT_DATA_FILE_TO_DB_PROCESS_COMPLETE_STATUS = False
                #         log_writer(script_name = 'Incident-Data-Process', steps = '11-K', status = 'ERROR', message = f'For Account: "{str(account_unique_id)}" Transferring Ticket From File To Database Process Completed But Worknotes Not Updated In ServiceNow')
                # except Exception as error:
                #     log_writer(script_name = 'Incident-Data-Process', steps = '11-K', status = 'ERROR', message = str(error))
                #     error_mail(file_name = 'Incident-Data-Process', step = '11-K', message = str(error))
    ######################################################
    ...
    if (INCIDENT_DATA_FILE_TO_DB_PROCESS_COMPLETE_STATUS):
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '12-A', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '12-B', message = str(error))
        ...
        try:
            if (str(incident_output_data_fill_backend_response['status']).lower() == 'error'):
                ...
                # sending error mail to developer
                # error_mail(file_name = str(incident_output_data_fill_backend_response['file_name']), step = str(incident_output_data_fill_backend_response['step']), message = str(incident_output_data_fill_backend_response['message']))
        except Exception as error:
            log_writer(script_name = 'Incident-Data-Process', steps = '12-C', status = 'ERROR', message = str(error))
            # error_mail(file_name = 'Incident-Data-Process', step = '12-C', message = str(error))
        ...
        if (str(incident_output_data_fill_backend_response['status']).lower() == 'success'):
            try:
                ...
                # elapsed_seconds = int((incident_output_data_fill_process_end_time - incident_output_data_fill_process_start_time) % 60) #type: ignore
                # elapsed_minutes = int(((incident_output_data_fill_process_end_time - incident_output_data_fill_process_start_time) % 3600) // 60) #type: ignore
                # elapsed_hours = int((incident_output_data_fill_process_end_time - incident_output_data_fill_process_start_time) // 3600) #type: ignore
                # # define servicenow worknotes
                ...
                #     INCIDENT_DATA_OUTPUT_DATA_FILL_PROCESS_COMPLETE_STATUS = True
                #     log_writer(script_name = 'Incident-Data-Process', steps = '12-D', status = 'SUCCESS', message = f'For Account: "{str(account_unique_id)}" Output Data Fill Process Completed And Worknotes Updated In ServiceNow')
                # else:
                #     INCIDENT_DATA_OUTPUT_DATA_FILL_PROCESS_COMPLETE_STATUS = False
                #     log_writer(script_name = 'Incident-Data-Process', steps = '12-D', status = 'ERROR', message = f'For Account: "{str(account_unique_id)}" Output Data Fill Process Completed But Worknotes Not Updated In ServiceNow')
            except Exception as error:
                log_writer(script_name = 'Incident-Data-Process', steps = '12-D', status = 'ERROR', message = str(error))
                # error_mail(file_name = 'Incident-Data-Process', step = '12-D', message = str(error))
    ######################################################
    ...
    if (INCIDENT_DATA_OUTPUT_DATA_FILL_PROCESS_COMPLETE_STATUS):
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '13-A', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '13-B', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '13-C', message = str(error))
        ...
        if (str(normalized_data_backend_response['status']).lower() == 'success'):
            try:
                ...
                #     INCIDENT_DATA_DATA_NORMALIZED_PROCESS_COMPLETE_STATUS = True
                #     log_writer(script_name = 'Incident-Data-Process', steps = '13-D', status = 'SUCCESS', message = f'For Account: "{str(account_unique_id)}" Data Normalization Process Completed And Worknotes Updated In ServiceNow')
                # else:
                #     INCIDENT_DATA_DATA_NORMALIZED_PROCESS_COMPLETE_STATUS = False
                #     log_writer(script_name = 'Incident-Data-Process', steps = '13-D', status = 'ERROR', message = f'For Account: "{str(account_unique_id)}" Data Normalization Process Completed But Worknotes Not Updated In ServiceNow')
            except Exception as error:
                log_writer(script_name = 'Incident-Data-Process', steps = '13-D', status = 'ERROR', message = str(error))
                # error_mail(file_name = 'Incident-Data-Process', step = '13-D', message = str(error))
    ######################################################
    ...
    if (INCIDENT_DATA_DATA_NORMALIZED_PROCESS_COMPLETE_STATUS):
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '14-A', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '14-B', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '14-C', message = str(error))
        ...
        if (str(information_clean_backend_response['status']).lower() == 'success'):
            try:
                ...
                #     INCIDNET_DATA_INFORMATION_CLEAN_PROCESS_COMPLETE_STATUS = True
                #     log_writer(script_name = 'Incident-Data-Process', steps = '14-D', status = 'SUCCESS', message = f'For Account: "{str(account_unique_id)}" Information Clean Process Completed And Worknotes Updated In ServiceNow')
                # else:
                #     INCIDNET_DATA_INFORMATION_CLEAN_PROCESS_COMPLETE_STATUS = False
                #     log_writer(script_name = 'Incident-Data-Process', steps = '14-D', status = 'ERROR', message = f'For Account: "{str(account_unique_id)}" Information Clean Process Completed But Worknotes Not Updated In ServiceNow')
            except Exception as error:
                log_writer(script_name = 'Incident-Data-Process', steps = '14-D', status = 'ERROR', message = str(error))
                # error_mail(file_name = 'Incident-Data-Process', step = '14-D', message = str(error))
    ######################################################
    ...
    if (INCIDNET_DATA_INFORMATION_CLEAN_PROCESS_COMPLETE_STATUS):
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '15-A', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '15-B', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '15-C', message = str(error))
        ...
        if (str(keywords_analysis_backend_response['status']).lower() == 'success'):
            try:
                ...
                #     INCIDENT_DATA_KEYWORD_ANALYSIS_PROCESS_COMPLETE_STATUS = True
                #     log_writer(script_name = 'Incident-Data-Process', steps = '15-D', status = 'SUCCESS', message = f'For Account: "{str(account_unique_id)}" Keywords Analysis Process Completed And Worknotes Updated In ServiceNow')
                # else:
                #     INCIDENT_DATA_KEYWORD_ANALYSIS_PROCESS_COMPLETE_STATUS = False
                #     log_writer(script_name = 'Incident-Data-Process', steps = '15-D', status = 'ERROR', message = f'For Account: "{str(account_unique_id)}" Keyword Analysis Process Completed But Worknotes Not Updated Into ServiceNow')
            except Exception as error:
                log_writer(script_name = 'Incident-Data-Process', steps = '15-D', status = 'ERROR', message = str(error))
                # error_mail(file_name = 'Incident-Data-Process', step = '15-D', message = str(error))
    ######################################################
    ...
    if (INCIDENT_DATA_KEYWORD_ANALYSIS_PROCESS_COMPLETE_STATUS):
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '16-A', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '16-B', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '16-C', message = str(error))
        ...
        if (str(automation_mapping_backend_response['status']).lower() == 'success'):
            try:
                ...
                #     INCIDENT_DATA_AUTOMATION_MAPPING_PROCESS_COMPLETE_STATUS = True
                #     log_writer(script_name = 'Incident-Data-Process', steps = '16-D', status = 'SUCCESS', message = f'For Account: "{str(account_unique_id)}" Automation Mapping Process Completed And Worknotes Updated Into SerivceNow')
                # else:
                #     INCIDENT_DATA_AUTOMATION_MAPPING_PROCESS_COMPLETE_STATUS = False
                #     log_writer(script_name = 'Incident-Data-Process', steps = '16-D', status = 'ERROR', message = f'For Account: "{str(account_unique_id)}" Automation Mapping Process Completed But Worknotes Not Updated In ServiceNow')
            except Exception as error:
                log_writer(script_name = 'Incident-Data-Process', steps = '16-D', status = 'ERROR', message = str(error))
                # error_mail(file_name = 'Incident-Data-Process', step = '16-D', message = str(error))
    ######################################################
    ...
    if (INCIDENT_DATA_AUTOMATION_MAPPING_PROCESS_COMPLETE_STATUS):
        ...
        try:
            # esoar_analysis_function_response = incident_esoar_process(account_unique_id = str(account_unique_id), ticket_number = str(servicenow_ticket_create_response['ticket_number']), ticket_sys_id = str(servicenow_ticket_create_response['sys_id']))
    ...
    if (INCIDENT_DATA_ESOAR_ANALYSIS_PROCESS_COMPLETE_STATUS):
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '17-A', message = str(error))
        ...
        try:
            ...
            # error_mail(file_name = 'Incident-Data-Process', step = '17-B', message = str(error))
    #####################################################

    # # updating "file_process_status" from "Pending" to "Complete":S18
    # if (INCIDENT_DATA_ESOAR_ANALYSIS_PROCESS_COMPLETE_STATUS):
    #     try:
    #         submitted_file_details_update_sql = '''
    #         UPDATE submitted_file_details
    #         SET file_process_status = 'Complete'
    #         WHERE account_unique_id = %s
    #         AND file_unique_id = %s
    #         RETURNING id;'''
    #         with psycopg2.connect(**database_connection_parameter) as database_connection: #type: ignore
    #             with database_connection.cursor() as database_cursor:
    #                 database_cursor.execute(submitted_file_details_update_sql, (str(account_unique_id), str(file_unique_id)))
    #                 submitted_file_details_update_result = database_cursor.fetchone()
    #                 if (submitted_file_details_update_result and submitted_file_details_update_result[0]):
    #                     INCIDNET_DATA_FILE_PROCESS_STATUS_UPDATE_PROCESS_COMPLETE_STATUS = True
    #                     log_writer(script_name = 'Incident-Data-Process', steps = '18', status = 'SUCCESS', message = f'For Account: "{account_unique_id}" File: "{file_unique_id}" "file_process_status" Updated To "Complete"')
    #                 else:
    #                     INCIDNET_DATA_FILE_PROCESS_STATUS_UPDATE_PROCESS_COMPLETE_STATUS = False
    #                     log_writer(script_name = 'Incident-Data-Process', steps = '18', status = 'ERROR', message = f'For Account: "{account_unique_id}" File: "{file_unique_id}" "file_process_status" Not Updated To "Complete"')
    #                     # sending error mail to developer
    #                     error_mail(file_name = 'Incident-Data-Process', step = '18', message = f'For Account: "{account_unique_id}" File: "{file_unique_id}" "file_process_status" Not Updated To "Complete"')
    #     except Exception as error:
    #         log_writer(script_name='Incident-Data-Process', steps='18', status='ERROR', message=str(error))
    #         error_mail(file_name = 'Incident-Data-Process', step = '18', message = str(error))
    # #####################################################

    # ##### send mail to user after complete process:S19 #####
    # if (INCIDNET_DATA_FILE_PROCESS_STATUS_UPDATE_PROCESS_COMPLETE_STATUS):
    #     try:
    #         # define email body
    #         email_body = f'''
    #         <html>
    ...
    #                     <p class="greeting"><strong>Hello Team,</strong></p>
    #                     <p><span style="color: green; font-weight: bold;"><strong>Success!</strong></span></p>
    #                     <p><strong>ESOAR Analysis process has been completed and published in the Power BI Dashboard. Please wait 1-2 hours before opening the Power BI Dashboard, as it takes some time for the backend to refresh.</strong> For all future references and tracking, please use the ticket number below:</p>
    #                     <p class="highlight">{str(servicenow_ticket_create_response['ticket_number'])}</p>
    #                     <p class="closing"> <strong>Thank you</strong> for your attention.<br><br>
    #                     Best Regards,<br>
    #                     <strong>Automation ASTRA</strong></p>
    #                     <p class="footer-note">
    ...
    #         # sending the email
    #         mail_handler(email_subject = 'ASTRA | ESOAR Analysis', email_to = str(user_email).lower(), email_cc = 'soumalya.mondal@capgemini.com;anurag.thakur@capgemini.com', email_body = str(email_body))
    #         log_writer(script_name='Incident-Data-Process', steps='19', status='SUCCESS', message=f'''Email Sent To User: "{str(user_email).lower()}" With ServiceNow Ticket: "{servicenow_ticket_create_response['ticket_number']}" Details After Complete File Processing''')
    #     except Exception as error:
    #         log_writer(script_name='Incident-Data-Process', steps='19', status='ERROR', message=str(error))
    #         error_mail(file_name = 'Incident-Data-Process', step = '19', message = str(error))
    # ########################################################